...
import { API_BASE_URL, API_ENDPOINTS_Banner } from "../../../services/api_url";
...
export const AddBanner = () => {
  ...
  const fetchDetails = async (data) => {
    const result = await axios.get(
      ...
      `${API_ENDPOINTS_Banner.get_by_id}?Id=${data}`
    );
    setInfo(result.data);
    ...
    try {
      ...
      if (id) {
        const response = await axiosInstance.post(
          ...
          `${API_ENDPOINTS_Banner.update_file}`
          ,
        ...
        const response = await axiosInstance.post(
          ...
          `${API_ENDPOINTS_Banner.upload_file}`,
          data
        );
  ...
  const BaseImageURL = "http://back.persianjewishhistory.com/";
