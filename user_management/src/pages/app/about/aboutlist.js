...
export const AboutList = () => {
  ...
  const getAllABOUTDetails = async () => {
    setIsloading(true);
    const result = await axiosInstance.post(
      `${API_ENDPOINTS_About.get_all_about}`
    );
  ...
  const handleEdit = (id) => {
    history.push(`/AddAbout/${id}`);
  };
  const handleDetail = (id) => {
    history.push(`/AboutDetail/${id}`);
  };
  const sweetalrt = (id) => {
    swal({
      title: "Are you sure?",
    ...
    try {
      axiosInstance
        .post(
           ...
           `${API_ENDPOINTS_About.delete_about}?id=${id}`
        )
        .then((res) => {
          ...
          <Table sx={{ width: "1250px" }} aria-label="customized table">
            <TableHead>
              <TableRow>
                ...
                <StyledTableCell> AboutUsPersian</StyledTableCell>
                <StyledTableCell >AboutUsEnglish</StyledTableCell>
                <StyledTableCell   >
                  Actions
                </StyledTableCell>
              </TableRow>
