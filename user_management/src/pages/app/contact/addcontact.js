...
export const Addcontact = () => {
  ...
  const fetchDetails = async (data) => {
    const result = await axios.post(
      ...
      `${API_ENDPOINTS_Contact.get_id_file}?Id=${data}`
    );
    const ids = id;
  ...
  const AddCnt = async (data) => {
    const result = await axios.post(
      // "https://localhost:44379/api/Contact/Add"
      `${API_ENDPOINTS_Contact.upload_file}`,
      data
    );
  ...
  const formik = useFormik({
    ...
    onSubmit: (values) => {
      ...
      console.log("ListDataShow>>>", upDateFieldLIst)
      const queryObj = {
        id: Number(id || 0),
  ...
  const onHandleRemove = async (contactID) => {
    if (id) {
      const result = await axios.post(
        // `https://localhost:44379/api/Contact/Delete-Contact-By-Number-Id?id=${contactID}`
        `${API_ENDPOINTS_Contact.deleteNumberById}?id=${contactID}`
      );
