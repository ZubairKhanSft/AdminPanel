...
export const AddUpcomingEvent = () => {
  ...
  //const ImageUrl = "https://localhost:44379/";
  const ImageUrl = "http://back.persianjewishhistory.com/";
  ...
  const fetchDetails = async (data) => {
    const result = await axios.get(
      ...
      `${API_ENDPOINTS_UpcomingEvent.get_id_event}?Id=${data}`
    );
    setInfo(result.data);
  ...
  async function makePostRequest(queryObj) {
    ...
    try {
      const data = {
        ...
        ticketPrice: queryObj?.ticketPrice,
