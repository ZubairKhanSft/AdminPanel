...
import { API_ENDPOINTS_Event } from "../../../services/api_url";
...
export const AddEvent = () => {
  ...
  const addEventValidation = Yup.object().shape({
    ...
    filePDF: Yup.string().required("Required"),
  });
  ...
  const fetchDetails = async (data) => {
    const result = await axios.get(
      ...
      `${API_ENDPOINTS_Event.get_id_file}?Id=${data}`
    );
    setInfo(result.data);
    ...
    <Grid>
      <Paper elevation={0} style={paperStyle}>
        <Grid align="center" sx={{ mt: 3 }}>
          <h2>{id ? "Update" : "Add New"}Jews Event</h2>
        </Grid>
        ...
        <form onSubmit={handleSubmit}>
          <TextField
            ...
            helperText={Boolean(errors.eventName) && errors.eventName}
          />
          ...
          <div style={{ marginTop: "20px" }}>
            ...
            {errors.filePDF && <p style={{ fontSize: 12 }}>Required</p>}
          </div>
